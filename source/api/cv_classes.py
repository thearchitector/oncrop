...
class ProcessingEngine:
	...
	def set_face(self, filename):
		...
		self.face_size_y, self.face_size_x, _ = self.face.shape
	...
	def get_frame(self):
		...
		if len(corners) == 1:
			...
			for corner in corners[0][0]:
				...
				ratio = self.face_size_y / self.face_size_x
				...
				if x2 - x1 >= frame_x or y2 - y1 >= frame_y:
					cv2.putText(frame, "You are too close to the frame", (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
				else:
					...
					elif self.file_type == "jpg": frame[y1:y2, x1:x2] = face
